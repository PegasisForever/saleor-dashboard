...
```typescript
urlJoin(window.location.origin, getAppMountUriForRedirect(), orderPath(id).slice(1));
```
...
- File: `./src/orders/components/OrderCopyLinkButton/OrderCopyLinkButton.stories.tsx` — state stories + `InTopNav` placement story

...
- [ ] Component Storybook stories cover `Default`, `Hover`, `Focus`, `Active`, `Disabled`, and `Copied` states with visually distinct rendering at settled Storybook load (decorators or `showCopiedState` arg — no reliance on transient `play` focus/hover/click simulation)
